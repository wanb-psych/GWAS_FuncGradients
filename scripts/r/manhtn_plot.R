...
for (x in list("g1","g2","g3")) {
  my_data <- fread(paste("/home/hpcwan1/rds/hpc-work/project/gwas_fg/results/GWAS/similarity/",mode,"/",mode,"_",x,"_GWAS.txt.gz",sep=""))
  ...
  CMplot(plot, plot.type="m", col=c("grey30","grey60"), LOG10=TRUE, ylim=c(3,11), threshold=c(5e-8,5e-5),
         ...
         file="png",file.name=paste(mode,"_",x,sep=""),dpi=300,file.output=TRUE,verbose=TRUE,width=8,height=4)
  CMplot(plot,plot.type="q",box=FALSE,file="png",file.name=paste(mode,"_",x,sep=""),dpi=300,
